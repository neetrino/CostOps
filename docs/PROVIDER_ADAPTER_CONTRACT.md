...
10. FIXED Hetzner: for each UTC month that **overlaps** the fetch range and is on/after `fixedEffectiveOn`, emit one `sourceType=FIXED` row **per UTC day**. Split `$fee` across days in that month (last day absorbs remainder so the month still sums to the operator amount). Do not dump the whole fee on the 1st. Amount edits rewrite the current month only. A leftover 1st-of-month lump is rewritten as daily rows using that booked month amount.
