# CostOps documentation

Technical docs are **English**. The source spec may be Russian.

| Doc | Purpose |
|-----|---------|
| [Neetrino_CostOps_CURSOR_SPEC.md](./Neetrino_CostOps_CURSOR_SPEC.md) | Product + implementation spec |
| [BRIEF.md](./BRIEF.md) | Short intake |
| [TECH_CARD.md](./TECH_CARD.md) | Stack and limits |
| [01-ARCHITECTURE.md](./01-ARCHITECTURE.md) | System shape |
| [02-TECH_STACK.md](./02-TECH_STACK.md) | Versions |
| [03-STRUCTURE.md](./03-STRUCTURE.md) | Folders and imports |
| [04-API.md](./04-API.md) | HTTP contract |
| [05-DATABASE.md](./05-DATABASE.md) | DB operations |
| [DATA_MODEL.md](./DATA_MODEL.md) | Prisma proposal |
| [PROVIDER_ADAPTER_CONTRACT.md](./PROVIDER_ADAPTER_CONTRACT.md) | Adapter interface |
| [ALERT_RULES.md](./ALERT_RULES.md) | Telegram spend engine |
| [CREDENTIAL_ROTATION.md](./CREDENTIAL_ROTATION.md) | Token expiry / 401 alerts + rotate links |
| [EXISTING_NEON_AUDIT.md](./EXISTING_NEON_AUDIT.md) | What to reuse from Neon |
| [MIGRATION_MAP.md](./MIGRATION_MAP.md) | Old → new entities/files |
| [OLD_NEON_DATABASE.md](./OLD_NEON_DATABASE.md) | `OLD_NEON_PROJECT_DATABASE_URL` — read-only neetrino/neon DB |
| [DESIGN.md](./DESIGN.md) | Visual/UX bar; design every detail |
| [IMPLEMENTATION_PLAN.md](./IMPLEMENTATION_PLAN.md) | Phases |
| [PROGRESS.md](./PROGRESS.md) | Status |
| [DECISIONS.md](./DECISIONS.md) | ADR index |
| [QUALITY_AUTOMATION_PLAN.md](./QUALITY_AUTOMATION_PLAN.md) | CI when the app exists |

`docs/reference/` is the template knowledge base (payments, generic checklists). It is not CostOps product spec.
