...
import { loadUsageTotals } from '@/features/usage';
...
export async function GET(request: Request): Promise<NextResponse> {
  ...
  try {
    return NextResponse.json(
      await cacheDashboardRead('usage-totals', dashboardReadCacheKey(parsed.query), () =>
        loadUsageTotals(parsed.query),
      ),
    );
  } catch (error) {
    return dashboardReadFailed('Usage totals read', error);
  }
}
