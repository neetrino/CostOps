...
export function normalizeTotals(raw: RawTotals, periodHours: number): NormalizedTotals {
  ...
  return {
    ...
    storageGbMonths,
    historyGbMonths,
    storageAvgGb: storageByteHours / periodHoursSafe / BYTES_PER_DECIMAL_GB,
    historyAvgGb: historyByteHours / periodHoursSafe / BYTES_PER_DECIMAL_GB,
    publicTransferGb: publicBytes / BYTES_PER_DECIMAL_GB,
    privateTransferGb: privateBytes / BYTES_PER_DECIMAL_GB,
    networkTotalGb: publicBytes / BYTES_PER_DECIMAL_GB + privateBytes / BYTES_PER_DECIMAL_GB,
...
/**
 * Org-wide 100 GB public transfer allowance, same math as neetrino/neon.
 */
export function applyPublicTransferAllowance(
  ...
  const totalPublicGb = projects.reduce(
    (sum, project) => sum + project.estimatedCost.publicTransferGb,
    0,
  );
  ...
  for (const project of projects) {
    const billableGb = project.estimatedCost.publicTransferGb * ratio;
    const publicTransferUsd = billableGb * rates.publicTransferPerGbUsd;
    project.estimatedCost.publicTransferBillableGb = billableGb;
    project.estimatedCost.publicTransferUsd = publicTransferUsd;
    project.estimatedCost.totalUsd =
      project.estimatedCost.computeUsd +
      project.estimatedCost.storageUsd +
      project.estimatedCost.historyUsd +
      project.estimatedCost.privateTransferUsd +
      project.estimatedCost.branchesUsd +
      publicTransferUsd;
  }
}
