...
import { INSTALL_BANNER_STORAGE_KEY } from '@/config/constants';
import {
  isInstallBannerDismissed,
  isStandaloneDisplay,
  readIosStandaloneFlag,
  shouldOfferIosInstall,
} from '@/shared/pwa/install-app';
...
function readDismissed(): boolean {
  try {
    return isInstallBannerDismissed(window.localStorage.getItem(INSTALL_BANNER_STORAGE_KEY));
  } catch {
    return false;
  }
}
...
/**
 * Whether to show the Add to Home Screen hint. Chromium uses the browser
